...
- The local operator flow is `npm install`, `npm run bootstrap`, then `npm run pm2:start`; the manifest-style entrypoints remain `scripts/build-all.sh` and `scripts/start-all.sh runtime`, and they should use the bundled PM2 binaries from `workspace/node_modules/.bin` without requiring a global PM2 install. Local daemon starts now go through `scripts/pm2-ensure.sh`, which resurrects a saved PM2 process list when present and otherwise boots from `ecosystem.config.cjs`, then saves the resulting process list for the next restart.
